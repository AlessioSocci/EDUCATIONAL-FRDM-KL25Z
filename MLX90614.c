...
void I2C0_IRQHandler(void)
{
	if(internalState == 1)
	{
		...
		{
			I2C0->D = MLX90614_REGISTER_TAMBIENT;	// send command access to RAM or EEPROM
	...
	if(internalState == 10)
	{
		...
		crc = crc8Coder(crc, MLX90614_REGISTER_TAMBIENT);
	...
	if(internalState == 2)
	{
		if((I2C0->S & (1 << 0)) == 0) // wait to receive ack from slave device
		{
			I2C0->D = MLX90614_REGISTER_TOBJECT1;	// send command access to RAM or EEPROM
			while((I2C0->S & (1 << 7)) == 0); // wait transfer complete
	...
	if(internalState == 20)
	{
		...
		uint8_t crc = crc8Coder(0, MLX90614_WRITEADDRESS);
		crc = crc8Coder(crc, MLX90614_REGISTER_TOBJECT1);
		crc = crc8Coder(crc, MLX90614_READADDRESS);
		crc = crc8Coder(crc, low);
		crc = crc8Coder(crc, high); 		// Received PEC control
	...
	if(internalState == 3)
	{
		if((I2C0->S & (1 << 0)) == 0) // wait to receive ack from slave device
		{
			I2C0->D = MLX90614_REGISTER_TOBJECT2;	// send command access to RAM or EEPROM
			while((I2C0->S & (1 << 7)) == 0); // wait transfer complete
	...
	if(internalState == 30)
	{
		...
		uint8_t crc = crc8Coder(0, MLX90614_WRITEADDRESS);
		crc = crc8Coder(crc, MLX90614_REGISTER_TOBJECT2);
		crc = crc8Coder(crc, MLX90614_READADDRESS);
		crc = crc8Coder(crc, low);
		crc = crc8Coder(crc, high); 		// Received PEC control
...
float convert_To_Temperature(uint16_t value, MLX90614_Scale scale)
{
    ...
    switch(scale)
    {
    	...
    	case MLX90614_SCALE_FARENHEIT:
    		temp = (1.8 * ((((float)value) * 0.02) - 273.15)) +  32.0;
        break;
    }
    return temp;
}

uint16_t MLX90614_getTemp(MLX90614_Scale scale, MLX90614_RAM_Or_EEPROM_Read_Address reg)
{
	if(reg == MLX90614_REGISTER_TAMBIENT)
	{
		delay(10);
		internalState = 1;
	}
	else if(reg == MLX90614_REGISTER_TOBJECT1)
	{
		delay(10);
		internalState = 2;
	}

	else if(reg == MLX90614_REGISTER_TOBJECT2)
	{
		delay(10);
		internalState = 3;
	}
	...
	convert_To_Temperature (value, scale);
