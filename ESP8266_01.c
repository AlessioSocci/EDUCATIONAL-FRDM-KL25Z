...
void ESP8266_queryGetListAP(char dataOut[]) // Only in client or both mode
{
	...
	UART_getMessage(dataIn2);
	UART_getMessage(dataIn3);
	UART_getMessage(dataIn4);
	UART_getMessage(dataIn5);
	UART_getMessage(dataIn6);
...
void ESP8266_queryWifiMode(char dataOut[])
{
	...
	delay(10);
	UART_send_AT_command(AT_querySetAP);
	UART_send_AT_CarriageReturn_LineFeed();
...
void ESP8266_joinAP(char dataOut[], char SSID[], char pass[])
{
	...
	delay(10);
	UART_send_AT_command(AT_joinAP);
	UART_send_AT_CarriageReturn_LineFeed();
...
void ESP8266_setCipMux(char dataOut[], uint8_t single_multi)
{
	...
	if (single_multi == 0)
	{
		...
		delay(10);
		UART_send_AT_command(AT_cipMux);
		UART_send_AT_CarriageReturn_LineFeed();
	}
...
void ESP8266_setCipServer(char dataOut[], uint8_t open_close)
{
	...
	if (open_close == 0)
	{
		...
		delay(10);
		UART_send_AT_command(AT_cipServer);
		UART_send_AT_CarriageReturn_LineFeed();
	}
...
void ESP8266_cipSend(char dataOut[], char id[], char numberOfByte[]) 	// only in AP or both mode!
																		// Parameters:
																		...
																		// if CIPMUX <mode> = 1 ->  <id> ---- 0 - 4, <length> ---- .
{
	...
	delay(10);
	UART_send_AT_command(AT_cipSend);
	UART_send_AT_CarriageReturn_LineFeed();
...
void ESP8266_sendPing(char dataOut[], char ping[])
{
	...
	delay(10);
	UART_send_AT_command(AT_sendPing);
	UART_send_AT_CarriageReturn_LineFeed();
